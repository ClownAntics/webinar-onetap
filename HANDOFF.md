...
2b. **Zoom also rejects blank/whitespace last names** (code 300, tested) — the `"-"` placeholder is mandatory when unknown. The "Copy link for Yumer" button now includes `&ln=[[contact.last_name]]` so real names come through. ⚠️ **Merge tags are snake_case** (`contact.first_name` / `contact.last_name`) — `README-build-v3.md` specified camelCase, but only snake_case is proven to expand in Omnisend (Yumer's 2026-08-17 sends, 173 registrations, zero literal tags in the data).
